...
public class FileController {
    ...
    @PatchMapping("/updateFile/{fileId}")
    @ResponseBody
    public ResponseEntity<?> handleFileUpdate(@RequestParam("file") MultipartFile file, @PathVariable String fileId) throws IOException, NoSuchAlgorithmException {
        UploadFileResponse response = fileService.updateFile(file, fileId);
        if(response.getFileId() != null){
            return ResponseEntity.status(HttpStatus.OK).body(response);
        }else {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
